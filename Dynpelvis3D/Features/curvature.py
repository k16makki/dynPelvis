...
# Ref: "Estimating Curvatures and Their Derivatives on Triangle Meshes" by Szymon Rusinkiewicz (2004)  
# the main function is "curvatures_and_derivatives" and it 
# computes the curvature tensor and the principal curvatures at
# each vertex of a mesh given in a face vertex data structure FV 
# This code was developed during an engineering internship at INT lab
#
#/----Script Authors---------------------------------------------\
#|                                                               |
#|       A.Souani (intern engineer)     A.Bohi, (PhD)            |
#|    aymane.souani@grenoble-inp.org  amine.bohi@univ-amu.fr     |
#|                                                               |
#\---------------------------------------------------------------/
    ...
    for i in range(FV.faces.shape[0]):
        ...
        b = np.array(
            [np.dot(n2 - n1, t), np.dot(n2 - n1, B), np.dot(n0 - n2, t),
             np.dot(n0 - n2, B), np.dot(n1 - n0, t), np.dot(n1 - n0, B)])
